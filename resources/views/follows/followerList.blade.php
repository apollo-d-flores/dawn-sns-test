...
<div class="user-icon-list-area">
    ...
    <div class="user-icon-list">
        @if(is_null($followerList))
            <p>フォローしているユーザーがいません。</p>
        @else
            @foreach($followerList as $lists)
                <div class='user-icon'>
                    <a href="http://127.0.0.1:8000/other/profile/{{$lists->follower_id}}"><img src="{{asset('/storage/usersIcon/'.$lists->image)}}"></a>
                </div>
            @endforeach
        @endif
    </div>
...
<hr class="gray-w4">
<div class="post-area">
    @if(is_null($followerPost))
            <p>投稿がありません。</p>
    @else
        @foreach($followerPost as $posts)
        <div class="post-info">
            <div class="user-icon">
